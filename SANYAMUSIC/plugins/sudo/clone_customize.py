...
# ── /addstring — Clone assistant string session ──────────────────────
@app.on_message(filters.command("addstring") & filters.private)
async def add_string_session(client, message: Message):
    if not await _is_authorized(client, message):
        return await message.reply_text(
            "❌ This command is only for the <b>clone owner</b>.",
            parse_mode=enums.ParseMode.HTML,
    ...
    if not reply or not reply.text:
        return await message.reply_text(
            "❓ <b>How to use:</b>\n\n"
            "Reply to your <b>string session message</b> with <code>/addstring</code>\n\n"
            "Your assistant's string session will be saved and used for voice chat.",
            parse_mode=enums.ParseMode.HTML,
        )

    string_session = reply.text.strip()
    if len(string_session) < 50:
        return await message.reply_text(
            "❌ Invalid string session. Please send a valid Pyrogram string session.",
            parse_mode=enums.ParseMode.HTML,
        )
...
async def clone_stats(client, message: Message):
    ...
    if not (is_clone and bot_id):
        return await message.reply_text("❌ Not a clone bot.")

    from SANYAMUSIC.utils.clone_db import get_clone_served_chats, get_clone_served_users
    chats = await get_clone_served_chats(bot_id)
    users = await get_clone_served_users(bot_id)

    await message.reply_text(
        f"ᴄʜᴀᴛs: <b>{len(chats)}</b>\n"
        f"ᴜsᴇʀs: <b>{len(users)}</b>",
        parse_mode=enums.ParseMode.HTML,
    )
