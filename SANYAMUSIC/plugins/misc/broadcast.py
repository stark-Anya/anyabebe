...
from SANYAMUSIC.utils.formatters import alpha_to_int
...
async def braodcast_message(client, message, _):
    ...
    if is_clone and bot_id:
        ...
        await message.reply_text("📢 Broadcasting to this bot's groups...")
        sent = 0
        # Sirf is clone ke served chats mein broadcast
        clone_chats = await get_clone_served_chats(bot_id)
        for chat in clone_chats:
            try:
                if message.reply_to_message:
                    await client.forward_messages(chat["chat_id"], y, x)
                else:
                    await client.send_message(chat["chat_id"], text=query)
                sent += 1
    ...
    # ── Main bot broadcast (original logic) ─────────────────────────
    if message.reply_to_message:
        x = message.reply_to_message.id
        y = message.chat.id
    else:
