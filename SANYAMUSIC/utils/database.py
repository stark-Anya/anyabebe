...
from SANYAMUSIC import userbot
...
async def group_assistant(self, chat_id: int) -> int:
    ...
    # Clone bot ka custom assistant check karo
    try:
        from SANYAMUSIC.utils.clone_db import get_clone_assistant_client
        from SANYAMUSIC import clone_bot_clients
        # Kaunsa clone bot is chat mein active hai check karo
        for bot_id, clone_client in clone_bot_clients.items():
            try:
                # Is chat mein ye clone active hai?
                member = await clone_client.get_chat_member(chat_id, (await clone_client.get_me()).id)
                if member:
                    clone_assis = get_clone_assistant_client(bot_id)
                    if clone_assis:
                        return clone_assis
                    break
            except Exception:
